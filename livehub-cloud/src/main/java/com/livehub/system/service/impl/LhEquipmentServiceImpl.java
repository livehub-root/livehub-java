...
public class LhEquipmentServiceImpl implements ILhEquipmentService {
     ...
     */
    @Override
    public LoginDevice login(LhEquipment lhEquipment) {
        LhEquipment device = lhEquipmentMapper.login(lhEquipment.getEquipmentNo(),
            lhEquipment.getRegistrationCode());
        if (device == null) {
            return null;
        }
        LoginDevice loginDevice = new LoginDevice();
        loginDevice.setDevice(device);
        loginDevice.setToken(device.getToken());
        return loginDevice;
    }
}
